...
export default async function ViewHumor(){
    ...
    return (
        <div className="grid justify-items-center">
            ...
            <div className="bg-green-Dark h-96 w-[25rem] min-h-full">
                <table>
                    <thead>
                        <tr className=" ml-4 bg-white-Normalhover w-full my-6 grid gap-3 grid-cols-3 ">
                            <th>Humor</th>
                            <th>Quando</th>
                            <th>Controles</th>
                        </tr>
                    ...
                    {humores.map((humor: { humor: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined; quando: string | number | Date | dayjs.Dayjs | null | undefined; id: any; } ) => {
                        return (
                            <tbody className="grid grid-cols-1 ml-4 w-full">
                                <tr className="bg-white-Normalhover">
                                    <td className="pl-10">{humor.humor}</td>
                                    <td className="pl-20"> {dayjs(humor.quando).format('D[/]M[/]YY [as] HH[:]mm')} </td>
